...
class StoryGenerator:
    def __init__(self):
        ...
        self.active_generations = {}  # Dictionary to track active generations

    def generate_story(self, prompt, num_pages, user_id):
        # Check if a generation is already in progress for this user and prompt
        key = (user_id, prompt)
        if key in self.active_generations:
            yield json.dumps({"error": "A story generation is already in progress for this prompt."})
            return

        # Mark this generation as active
        self.active_generations[key] = True
        ...
        try:
            ...
            # Remove the key from active generations after completion
...
def save_story():
    ...
    if conn is not None:
        try:
            ...
            if existing_story:
                conn.rollback()  # Roll back the transaction if a duplicate is found
                return jsonify({"success": False, "error": "Story already exists."}), 400

            # Insert the new story if no duplicates
            cursor.execute("INSERT INTO stories (user_id, prompt, num_pages, style_prompt, character_descriptions) VALUES (?, ?, ?, ?, ?)",
