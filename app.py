...
from dateutil import parser  # You might need to install python-dateutil
...
def fetch_stories():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            # Fetch unique stories with their first image
            cursor.execute("""
                SELECT s.id, s.prompt, s.num_pages, s.created_at, s.user_id, 
                       (SELECT image_url FROM story_pages WHERE story_id = s.id ORDER BY page_number LIMIT 1) as first_image_url
                FROM stories s
            ...
            for row in rows:
                stories.append({
                    ...
                    'created_at': row[3],
                    'user_id': row[4],
                    'image_url': row[5]  # This will be the first image URL for the story
                })
        except Error as e:
...
def storyboard():
    stories = fetch_stories()  # Fetch stories using the improved function
    ...
    for story in stories:
        ...
        if 'created_at' in story and story['created_at']:
            if isinstance(story['created_at'], str):
                try:
                    ...
                    # If the format is different, try a more flexible parsing
                    story['created_at'] = parser.parse(story['created_at'])
        ...
        if conn:
            try:
                ...
                cursor.execute("SELECT username FROM users WHERE id = ?", (story['user_id'],))
                user_data = cursor.fetchone()
                story['creator_name'] = user_data[0] if user_data else "Unknown"
            except Error as e:
                print(f"Error fetching creator name: {e}")
                story['creator_name'] = "Unknown"
            finally:
                conn.close()
